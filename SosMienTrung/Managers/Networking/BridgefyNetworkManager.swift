...
final class BridgefyNetworkManager: NSObject, ObservableObject, BridgefyDelegate {
    ...
    /// SOS packets chờ broadcast khi Bridgefy sẵn sàng (BT bật) hoặc khi bridgefy.send thất bại
    private var pendingSOSBroadcasts: [(sosPacket: SOSPacket, message: String, timestamp: Date)] = []
    private let pendingBroadcastQueue = DispatchQueue(label: "bridgefy.pending.broadcast")
    /// Timer định kỳ để flush pendingSOSBroadcasts (Fix #7)
    ...
    /// True nếu đang trong quá trình soft-restart: bridgefy.stop() đã gọi và đang chờ bridgefyDidStop
    ...
    func start() {
        ...
        do {
            ...
            // [Fix #7] Timer định kỳ 5s flush pending SOS broadcasts (an toàn dù đã có trigger trong didConnect/didEstablishSecureConnection)
            startPendingFlushTimer()
        } catch {
            lifecycleQueue.sync {
    ...
    /// Buộc tái khởi tạo Bridgefy: stop() → chờ bridgefyDidStop → nil instance → start() lại.
    /// Dùng khi app foreground sau background để reset BLE central/peripheral –
    /// khắc phục tình trạng "Bridgefy already started" nhưng link BLE đã chết do iOS suspend.
    func softRestart() {
        #if targetEnvironment(simulator)
        return
        #else
        let (paused, currentlyStarting) = lifecycleQueue.sync {
            (!pauseReasons.isEmpty, isStartInProgress)
        }
        guard !paused else {
            print("⏸ Bridgefy softRestart skipped (paused)")
            return
        }
        guard !isIdentityDisabled else {
            print("ℹ️ Bridgefy softRestart skipped (identity disabled)")
            return
        }
        if currentlyStarting {
            print("⏳ Bridgefy softRestart skipped (start in progress)")
            return
        }
        if pendingRestartAfterStop {
            print("⏳ Bridgefy softRestart already in progress")
            return
        }

        if let bridgefy = self.bridgefy {
            print("🔄 Bridgefy soft restart: stopping current instance to refresh BLE...")
            pendingRestartAfterStop = true
            bridgefy.stop()
            // Safety timeout: nếu bridgefyDidStop không fire trong 3s, force restart
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
                guard let self = self, self.pendingRestartAfterStop else { return }
                print("⚠️ Bridgefy didStop did not fire within 3s – forcing restart")
                self.pendingRestartAfterStop = false
                self.bridgefy = nil
                self.start()
            }
        } else {
            print("🔄 Bridgefy soft restart: no live instance, starting fresh...")
            start()
        }
        #endif
    }
    ...
    @discardableResult
    func sendSosRequestToGeneralChat(_ sos: SosRequestDto) -> Bool {
        guard let bridgefy, let sender = bridgefy.currentUserId else {
            print("Bridgefy not started or missing userId")
            return false
        }
        ...
        let messageId = UUID()
        let timestamp = Date()
        let text = Self.generalChatText(for: sos)
        let payload = MessagePayload(
            type: .sosLocation,
            text: text,
            messageId: messageId,
        ...
        do {
            ...
            print("📤 Shared SOS #\(sos.id) to general chat")
            return true
        } catch {
            print("❌ Bridgefy SOS share failed: \(error.localizedDescription)")
            return false
        }
    }

    private static func generalChatText(for sos: SosRequestDto) -> String {
        var header = "SOS #\(sos.id)"
        ...
        var lines = [
            header,
            "Trạng thái: \(SosDisplayFormatter.localizedStatus(sos.status))"
        ]

        if let priority = SosDisplayFormatter.localizedPriority(sos.priorityLevel) {
            lines.append("Mức ưu tiên: \(priority)")
        }

        let summary = sos.msg.trimmingCharacters(in: .whitespacesAndNewlines)
        if !summary.isEmpty {
    ...
    func sendSOSWithUpload(_ text: String) async {
        ...
        // Broadcast qua mesh network để các device khác có thể relay; không tự đưa vào chat tổng.
        await MainActor.run {
    ...
    /// - Parameters:
    ///   - formData: Dữ liệu form SOS
    ///   - reusePacketId: Nếu khác nil, dùng lại packetId cõ (cho luồng resend) và KHÔNG tạo entry SavedSOS mới.
    ///     Entry cũ sẽ được cập nhật status/event trực tiếp vì id == packetId.
    /// - Returns: `true` nếu gửi thành công lên server, `false` nếu chỉ relay qua mesh
    func sendStructuredSOS(_ formData: SOSFormData, reusePacketId: String? = nil) async -> Bool {
        guard UserProfile.shared.currentUser != nil else {
        ...
        // Tạo Enhanced SOS Packet — nếu resend thì dùng lại packetId cũ để cuối cùng status update trúng entry cũ.
        let enhancedPacket: SOSPacketEnhanced
        if let reusePacketId = reusePacketId {
            let packet = formData.toSOSPacket(
                originIdOverride: sender,
                packetIdOverride: reusePacketId,
                timestampOverride: timestamp
            )
            enhancedPacket = SOSPacketEnhanced(packet: packet, priorityScore: formData.priorityScore)
        } else {
            enhancedPacket = SOSPacketEnhanced(
                from: formData,
                originId: sender,
                latitude: latitude,
                longitude: longitude
        ...
        // 📦 Lưu SOS vào storage — chỉ tạo entry mới khi KHÔNG phải resend
        if reusePacketId == nil {
            await MainActor.run {
        ...
        // [Fix #6] Bỏ duplicate broadcast: khi !serverReached, Gateway đã broadcast envelope .serverRequest (chứa enhanced).
        // Không tự append vào chat tổng; chỉ xử lý các trường hợp transport:
        //   - serverReached: phát thêm envelope .serverRequest cho peer biết
        //   - Bridgefy chưa sẵn sàng: enqueue vào pendingSOSBroadcasts
        let meshMessage = formData.toSOSMessage()
        if bridgefy != nil && bridgefy?.currentUserId != nil {
            await MainActor.run {
                if serverReached {
                    // Server đã ack: vẫn broadcast envelope qua mesh để peer biết/relay, không tạo bubble chat.
                    let envelope = ServerRequestEnvelope.enhancedSOS(enhancedPacket)
                    sendServerRequest(envelope)
                } else {
                    // !serverReached: Gateway.submitSOSEnhanced ở trên đã broadcast .serverRequest envelope qua mesh.
                    SOSStorageManager.shared.updateStatusWithEvent(
                        id: packetId,
                        status: .relayed,
            ...
            enqueuePendingSOSBroadcast(sosPacket: sosPacket, message: meshMessage, timestamp: timestamp)
    ...
    private func broadcastSOSPacket(_ sosPacket: SOSPacket, originalMessage: String, timestamp: Date) {
        guard let bridgefy, let sender = bridgefy.currentUserId else {
            // Bridgefy chưa sẵn sàng → enqueue để retry sau
            enqueuePendingSOSBroadcast(sosPacket: sosPacket, message: originalMessage, timestamp: timestamp)
            return
        }
        ...
        do {
            ...
            // [Fix #4] Send fail (vd: chưa secure connection) ⇒ enqueue để retry
            print("❌ Failed to broadcast SOS: \(error.localizedDescription) – enqueue retry")
            enqueuePendingSOSBroadcast(sosPacket: sosPacket, message: originalMessage, timestamp: timestamp)
        }
    }

    /// Đưa SOS vào hàng đợi broadcast – dùng khi Bridgefy chưa sẵn sàng hoặc bridgefy.send thất bại
    private func enqueuePendingSOSBroadcast(sosPacket: SOSPacket, message: String, timestamp: Date) {
        pendingBroadcastQueue.async {
            if !self.pendingSOSBroadcasts.contains(where: { $0.sosPacket.packetId == sosPacket.packetId }) {
                self.pendingSOSBroadcasts.append((sosPacket: sosPacket, message: message, timestamp: timestamp))
                print("📦 [SOS] Enqueued pending broadcast for packetId=\(sosPacket.packetId) (queue size=\(self.pendingSOSBroadcasts.count))")
            }
        }
    }
    ...
    /// Broadcast lại các SOS đã lưu khi Bridgefy chưa sẵn sàng / send trước đó thất bại
    func rebroadcastPendingSOS() {
        pendingBroadcastQueue.async { [weak self] in
            guard let self = self else { return }
            guard !self.pendingSOSBroadcasts.isEmpty else { return }
            guard self.bridgefy?.currentUserId != nil else {
                // Vẫn chưa sẵn sàng – để timer tick sau gọi lại
                return
            }

            let pending = self.pendingSOSBroadcasts
            self.pendingSOSBroadcasts.removeAll()
            ...
            for item in needBroadcast {
                DispatchQueue.main.async {
                    self.broadcastSOSPacket(item.sosPacket, originalMessage: item.message, timestamp: item.timestamp)
                    SOSStorageManager.shared.updateStatusWithEvent(
    ...
    /// [Fix #7] Timer định kỳ flush pending SOS broadcasts – phòng trường hợp
    /// trigger ở didConnect / didEstablishSecureConnection bị bỏ lỡ.
    private func startPendingFlushTimer() {
        guard pendingFlushTimer == nil else { return }
        let timer = DispatchSource.makeTimerSource(queue: pendingBroadcastQueue)
        ...
        print("⏱ [SOS] Pending flush timer started (5s interval)")
    }

    private func stopPendingFlushTimer() {
    ...
    func bridgefyDidStop() {
        ...
        // Nếu đang trong soft-restart → nil instance và start() lại
        if pendingRestartAfterStop {
            ...
            print("🔄 Bridgefy stopped → restarting fresh instance...")
    ...
    func bridgefyDidConnect(with userId: UUID) {
        ...
        // [Fix #3] Đẩy heartbeat ngay để peer mới biết level của mình thay vì chờ tick 8s
        MeshManager.shared.kickHeartbeatNow()
        ServerRequestGateway.shared.triggerRetry(reason: .peerUpdate)
        // [Fix #7] Có peer mới ⇒ flush queue SOS pending để đẩy đi sớm
    ...
    func bridgefyDidEstablishSecureConnection(with userId: UUID) {
        ...
        // [Fix #3] Lúc này mới thực sự có thể send qua peer này.
        // Kick heartbeat + flush pending SOS + trigger Gateway retry để các packet bị fail trước đó được phát lại.
        MeshManager.shared.kickHeartbeatNow()
        ServerRequestGateway.shared.triggerRetry(reason: .peerUpdate)
        rebroadcastPendingSOS()
    }
    ...
    func sendMeshData(_ data: Data, to peerId: String?, suppressTransportLog: Bool = false) {
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                ...
                // [Fix #4] Heartbeat sẽ retry tự nhiên ở tick tiếp theo (8s);
                // SOS qua MeshRouter đã enqueue offline khi không có next hop.
    ...
    func sendServerRequest(_ request: ServerRequestEnvelope) {
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                ...
                // [Fix #4] Send fail. Gateway retry timer sẽ gọi lại sendServerRequest sau (đợt 1 = 3s).
                print("[ServerRequest] ⚠️ Send fail (\(error.localizedDescription)). Will be retried by Gateway.")
            }
        }
    }
    ...
    func sendServerAck(_ ack: ServerRequestAck) {
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                ...
                // ACK sẽ được tự động gửi lại lần sau khi sender retry request và mình nhận completed-dedup
                print("[ServerRequest] ⚠️ ACK send fail (\(error.localizedDescription)). Will resend on next retry.")
            }
        }
    }
